...
var _ = Describe(`AccountManagementV4`, func() {
	...
	Describe(`GetAccount(getAccountOptions *GetAccountOptions) - Operation response error`, func() {
		getAccountPath := "/v4/accounts/account_id"
		...
		Context(`Using mock server endpoint with timeout`, func() {
			BeforeEach(func() {
				testServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
					...
					res.Header().Set("Content-type", "application/json")
					res.WriteHeader(200)
					fmt.Fprintf(res, "%s", `{"name": "Name", "id": "ID", "owner": "Owner", "owner_userid": "OwnerUserid", "owner_iamid": "OwnerIamid", "type": "Type", "status": "Status", "linked_softlayer_account": "LinkedSoftlayerAccount", "team_directory_enabled": "TeamDirectoryEnabled", "traits": {"eu_supported": false, "poc": false, "hippa": false}}`)
				}))
			})
